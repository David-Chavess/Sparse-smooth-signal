...
def plot_2_reconstruction(sparse: np.ndarray, smooth: np.ndarray, name: str = "") -> None:
    """
    ...
    im_p = ax1.imshow(sparse, vmin=0, vmax=SparseSmoothSignal.MAX_SPARSE_AMPLITUDE)
    ax1.axis('off')

    im_s = ax2.imshow(smooth, vmin=0, vmax=SparseSmoothSignal.MAX_SMOOTH_AMPLITUDE)
    ax2.axis('off')
